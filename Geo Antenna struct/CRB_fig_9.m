...
%         Nr      = Nr_UCA * Nr_ULA;        % ULA
    ...
    %============================================
        ...
        CRB_op(Nr_i)               = abs(trace(pinv(Iop_full)));
    %============================================
        %Only Pilot Specular   
        Iop_spec_ULA            = G_ULA*G_ULA'*Iop_full*G_ULA*G_ULA';
        CRB_op_ULA_spec(Nr_i)      = abs(trace(pinv(Iop_spec_ULA)));

        Iop_spec_UCyA           = G_UCyA*G_UCyA'*Iop_full*G_UCyA*G_UCyA';
        CRB_op_UCyA_spec(Nr_i)     = abs(trace(pinv(Iop_spec_UCyA)));
    end
...
Nr_i = 1:(128/16);
figure()
semilogy(Nr_i, CRB_op_ULA, '-k');
hold on;
semilogy(Nr_i, CRB_op_UCyA, '*');
semilogy(Nr_i, CRB_op_ULA_spec, '-g o');
semilogy(Nr_i, CRB_op_UCyA_spec, '-r d');
