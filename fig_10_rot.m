...
for t = 1:loop
    ...
    for rot_nor = rot_nor_f
        ...
        for Nr_ULA_index=1:Nr_ULA
            for Nr_UCA_index=1:Nr_UCA
                UCyA_elements_nor(1, Nr_ULA_index, Nr_UCA_index) = R_nor * sin((Nr_UCA_index-1)*(2*pi/Nr_UCA) + rot_nor * (Nr_ULA_index - 1) * (2*pi/Nr_UCA)) ;         % x
                UCyA_elements_nor(2, Nr_ULA_index, Nr_UCA_index) = R_nor * cos((Nr_UCA_index-1)*(2*pi/Nr_UCA) + rot_nor * (Nr_ULA_index - 1) * (2*pi/Nr_UCA)) ;         % y
    ...
    %============================================
        ...
        CRB_op(index)           = abs(trace(pinv(Iop_full)));
    %============================================
        %Only Pilot Specular   
        Iop_spec_ULA            = G_ULA*G_ULA'*Iop_full*G_ULA*G_ULA';
        try
            CRB_op_ULA_spec(index)  = abs(trace(pinv(Iop_spec_ULA)));
        catch ME
            t = t - 1;
            continue
        end
        ...
        try
            CRB_op_UCyA_spec(index) = abs(trace(pinv(Iop_spec_UCyA)));
        catch ME
            t = t - 1;
            continue
        end
...
%% figure
h = figure();
semilogy(rot_nor_f, CRB_op_ULA, '-b *', 'LineWidth', 1.5, 'Color', '#7E2F8E');
hold on;
semilogy(rot_nor_f, CRB_op_ULA_spec, '-g o', 'LineWidth', 1.5, 'Color', "#0072BD", 'MarkerFaceColor', "#FFFF00");
semilogy(rot_nor_f, CRB_op_UCyA_spec, '-r d', 'LineWidth', 1.5, 'Color', "#FF0000", 'MarkerFaceColor', "#FFFF00");

grid minor;
ylabel('CRB (dB)', 'FontSize', 14, 'Interpreter','latex');
xlabel('Rotated', 'FontSize', 14, 'Interpreter','latex');
