...
for t = 1:100
    ...
    for Nt = 1:4         % number of transmit antennas
    ...
    %============================================
        ...
        CRB_op(Nt)               = abs(trace(pinv(Iop_full)));
    %============================================
        %Only Pilot Specular   
        Iop_spec_ULA            = G_ULA*G_ULA'*Iop_full*G_ULA*G_ULA';
        CRB_op_ULA_spec(Nt)      = abs(trace(pinv(Iop_spec_ULA)));

        Iop_spec_UCyA           = G_UCyA*G_UCyA'*Iop_full*G_UCyA*G_UCyA';
        CRB_op_UCyA_spec(Nt)     = abs(trace(pinv(Iop_spec_UCyA)));
    end
...
Nt = 1:4;
figure()
semilogy(Nt, CRB_op_ULA, '-k');
hold on;
semilogy(Nt, CRB_op_UCyA, '*');
semilogy(Nt, CRB_op_ULA_spec, '-g o');
semilogy(Nt, CRB_op_UCyA_spec, '-r d');
