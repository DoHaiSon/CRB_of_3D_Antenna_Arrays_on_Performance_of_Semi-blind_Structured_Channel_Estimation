...
for t = 1:loop
    ...
    %============================================
        ...
        CRB_op(Nr_ULA)           = abs(trace(pinv(Iop_full)));
    %============================================
        %Only Pilot Specular   
        Iop_spec_ULA            = G_ULA*G_ULA'*Iop_full*G_ULA*G_ULA';
        try
            CRB_op_ULA_spec(Nr_ULA)  = abs(trace(pinv(Iop_spec_ULA)));
        catch ME
            t = t - 1;
            continue
        end
        ...
        try
            CRB_op_UCyA_spec(Nr_ULA) = abs(trace(pinv(Iop_spec_UCyA)));
        catch ME
            t = t - 1;
            continue
        end
...
%% figure
h = figure();
semilogy(N_r_ULA, CRB_op_ULA, '-b', 'LineWidth', 1.5, 'Color', '#7E2F8E');
hold on;
semilogy(N_r_ULA, CRB_op_UCyA, '*', 'LineWidth', 1.5, 'Color', "#0072BD", 'MarkerSize', 7);
semilogy(N_r_ULA, CRB_op_ULA_spec, '-g o', 'LineWidth', 1.5, 'Color', "#0072BD", 'MarkerFaceColor', "#FFFF00");
semilogy(N_r_ULA, CRB_op_UCyA_spec, '-r d', 'LineWidth', 1.5, 'Color', "#FF0000", 'MarkerFaceColor', "#FFFF00");
