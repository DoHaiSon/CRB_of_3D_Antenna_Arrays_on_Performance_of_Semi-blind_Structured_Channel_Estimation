...
for Nr_UCA = N_r_UCA
    Cyy_f{Nr_UCA} = gather(getfield(load(['./Cyy/NrUCA_5db_' num2str(Nr_UCA) '.mat']), 'Cyy_inv'));
end
...
%         Cyy      = sigmax2  * LAMBDA  * LAMBDA'   + sigmav2 * eye(K*Nr);
    ...
    %============================================
        ...
        CRB_SB_i           = pinv(I_SB);
        CRB_SB_ULA(Nr_UCA / 8)  = abs(trace(CRB_SB_i));

    %============================================
        %Semiblind Specular
        I_SB_spec               = G_ULA*G_ULA'*I_SB*G_ULA*G_ULA';
        CRB_SB_spec_i           = pinv(I_SB_spec);
        CRB_SB_ULA_spec(Nr_UCA / 8)  = abs(trace(CRB_SB_spec_i));   
 ...
 %% ========================================================================================
        ...
        CRB_SB_spec_i           = pinv(I_SB_spec);
        CRB_SB_UCyA_spec(Nr_UCA / 8) = abs(trace(CRB_SB_spec_i));
    end

    if (all(diff(CRB_SB_ULA) <= 1e-8) && all(diff(CRB_SB_ULA_spec) <= 1e-8) && all(diff(CRB_SB_UCyA_spec) <= 1e-8))
...
%% figure
h = figure();
semilogy(N_r_UCA, CRB_SB_ULA, '-b', 'LineWidth', 1.5, 'Color', '#7E2F8E');
hold on;
semilogy(N_r_UCA, CRB_SB_ULA_spec, '-g o', 'LineWidth', 1.5, 'Color', "#0072BD", 'MarkerFaceColor', "#FFFF00");
semilogy(N_r_UCA, CRB_SB_UCyA_spec, '-r d', 'LineWidth', 1.5, 'Color', "#FF0000", 'MarkerFaceColor', "#FFFF00");
